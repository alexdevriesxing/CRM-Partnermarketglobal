...
const requiredSenders = requiredDomains.map((domain) => `info@${domain}`);
...
if (String(process.env.EMAIL_DOMAINS_ONBOARDED || '').toLowerCase() !== 'true') {
  errors.push('EMAIL_DOMAINS_ONBOARDED must be set to true after both domains show Ready in Cloudflare Email Sending.');
}
...
if (mainConfig.vars?.AUTH_MODE !== 'access') errors.push('Production AUTH_MODE must be access.');
if (!mainConfig.vars?.OWNER_EMAIL) errors.push('OWNER_EMAIL is required for this personal CRM.');
...
for (const sender of requiredSenders) {
  if (!emailBinding?.allowed_sender_addresses?.includes(sender)) errors.push(`The Email Worker binding is missing allowed sender ${sender}.`);
}
