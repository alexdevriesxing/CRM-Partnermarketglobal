...
await update('src/intelligence.js', (content) => content
...
await update('public/intelligence.js', (content) => {
  let result = content
    ...
    .replace('aria-label="Commercial intelligence activity window"', 'aria-label="Opportunity inactivity risk window"');
  if (!result.includes("const navigationRiskBadge = document.querySelector('#intelligenceRiskCount')")) {
    result = result.replace("  root.innerHTML = `\n", "  const navigationRiskBadge = document.querySelector('#intelligenceRiskCount');\n  if (navigationRiskBadge) navigationRiskBadge.textContent = Number(data.summary?.urgent_deals || 0) || '';\n\n  root.innerHTML = `\n");
  }
  return result;
});
