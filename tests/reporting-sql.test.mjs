...
test('detailed reporting executes against the fully migrated CRM schema', async () => {
  ...
  database.prepare(`INSERT INTO email_sender_identities (id,workspace_id,email_address,display_name,domain,is_default,is_active,created_by) VALUES (?,?,?,?,?,?,?,?)`).run('s1','ws1','info@partnermarketglobal.com','PartnerMarket Global','partnermarketglobal.com',1,1,'u1');
  database.prepare(`INSERT INTO email_messages (id,workspace_id,sender_identity_id,organization_id,contact_id,user_id,from_email,to_json,subject,status,recipient_count,delivery_attempts,sent_at,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run('m1','ws1','s1','o1','c1','u1','info@partnermarketglobal.com','["buyer@example.com"]','Commercial follow-up','sent',1,1,iso(-1),iso(-1));
